...
static yo_token_t *yo_parse_consume(yo_parser_t *p, yo_token_type_t type)
{
...
static void yo_parse_consume_keyword(yo_parser_t *p, yo_string_t s)
{
...
static yo_string_t yo_parse_consume_name(yo_parser_t *p)
{
    yo_token_t *token = yo_parse_consume(p, YO_TOKEN_IDEN);
...
static yo_parsed_type_t yo_parse_type(yo_parser_t *p)
{
    yo_parsed_type_t ret = { 0 };
...
static yo_parsed_arg_t yo_parse_arg(yo_parser_t *p)
{
    yo_parsed_arg_t ret = { 0 };
...
static yo_parsed_func_t yo_parse_func(yo_parser_t *p)
{
    yo_parsed_func_t ret = { 0 };
    ...
    while (p->token->type != ')')
    {
        yo_parsed_arg_t *arg = yo_arena_push_struct(p->arena, yo_parsed_arg_t, false);
        *arg = yo_parse_arg(p);
...
static yo_parsed_enum_t yo_parse_enum(yo_parser_t *p)
{
    //
    ...
    while (p->token->type != '}')
    {
        yo_parsed_enum_member_t *member = yo_arena_push_struct(p->arena, yo_parsed_enum_member_t, true);
...
{
    yo_parser_t p = { .token = tokens, arena = arena };
    ...
    while (p.token->type != YO_TOKEN_EOF)
    {
            ...
            (yo_string_equal(p.token->text, yo_string("YO_PLATFORM_API"))))
        {
            ...
            yo_parsed_func_t *func = yo_arena_push_struct(p.arena, yo_parsed_func_t, false);
            *func = yo_parse_func(&p);
            yo_slist_add(&p.result.funcs, func);
        }
        else if (yo_string_equal(p.token->text, yo_string("enum")))
        {
            yo_parsed_enum_t *enum_ = yo_arena_push_struct(p.arena, yo_parsed_enum_t, false);
